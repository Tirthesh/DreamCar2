            ...
            <table class="table table-bordered table-striped {{ count($products) > 0 ? 'datatable' : '' }} @can('product_delete') dt-select @endcan">
                <thead>
                    <tr>
                        ...
                        <th>@lang('quickadmin.products.fields.make')</th>
                        <th>@lang('quickadmin.products.fields.model')</th>
                        <th>@lang('quickadmin.products.fields.description')</th>
                        <th>@lang('quickadmin.products.fields.keywords')</th>
                    ...
                    @if (count($products) > 0)
                        @foreach ($products as $product)
                            <tr data-entry-id="{{ $product->id }}">
                                ...
                                <td field-key='make'>{!! $product->make !!}</td>
                                <td field-key='model'>{!! $product->model !!}</td>
                                <td field-key='description'>{!! $product->description !!}</td>
                                <td field-key='keywords'>{{ $product->keywords }}</td>
